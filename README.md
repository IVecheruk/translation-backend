...
| `GET` | `/api/account/subscription` | JWT | текущее состояние подписки |
...
Backend сверяет подписанные провайдером сумму, валюту, период и идентификатор
продукта с неизменяемым снимком заявки. Повторные события безопасно
дедуплицируются. Для Tribute UUID заказа является идентификатором полного
платёжного lifecycle; event ID, order ID, customer ID и subscription ID
хранятся раздельно и не возвращаются клиенту.

Продления, неуспешные списания, восстановление после оплаты, отмена и возврат
синхронизируются webhook-событиями. Пропущенное окончание оплаченного периода
страхуется локальным планировщиком. Просроченные заявки и старые технические
платёжные записи очищаются ограниченными пакетами согласно retention-политике.
